...
async def test_session_middleware(request_mock):
    ...
    with patch("src.main.middlewares.session.get_or_set_db_session", return_value=(None, db_session_mock)):
        ...
        (await db_session_mock.__aenter__()).commit.assert_called_once()
...
async def test_session_middleware_error_handling(request_mock):
    ...
    with patch("src.main.middlewares.session.get_or_set_db_session", return_value=(None, db_session_mock)):
        ...
        (await db_session_mock.__aenter__()).rollback.assert_called_once()
