...
for service in ${SERVICES[@]}; do
  ...
  generateCertsPostgres "client" "$service" "postgres,${service}" "postgresdb" "$DIR/$service" # Client PostgresDB

  if [ "$service" = "aec-back" ]; then
    # Create JWT certs
...
# Generate certificates for PostgreDB
echo "$COUNT - Generating certificates for the \"PostgreDB Server\"..."
generateCertsPostgres "server" "postgres" $SERVICES_ALT_NAMES_MONGO "server" "$DIR/postgres"
