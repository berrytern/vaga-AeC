...
async def set_book_as_favorite(request: Request, reader_id: UUID, book_id: UUID):
    response = await DI_CONTAINER.controllers.favorite_controller().create(
        reader_id, book_id
    )
    return JSONResponse(
...
async def get_all_favorite_books_of_reader(request: Request, reader_id: UUID):
    ...
    response = await DI_CONTAINER.controllers.favorite_controller().get_all(query)
    return JSONResponse(
        content=response[0], status_code=response[1], headers=response[2]
    )
...
async def get_one_favorite_book_of_reader(
    ...
    response = await DI_CONTAINER.controllers.favorite_controller().get_one(
        reader_id, book_id
    )
    return JSONResponse(
        content=response[0], status_code=response[1], headers=response[2]
...
async def delete_one_favorite_book_of_reader(
    ...
    response = await DI_CONTAINER.controllers.favorite_controller().delete_one(
        reader_id, book_id
    )
    return JSONResponse(
        content=response[0], status_code=response[1], headers=response[2]
